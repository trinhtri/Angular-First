...
export class ChatBarComponent extends AppComponentBase implements OnInit, AfterViewInit {
    ...
    scrollToBottom(): void {
        setTimeout(() => {
            const $scrollArea = $('.page-quick-sidebar-chat-user-messages');
    ...
    bindUiEvents(): void {
        ...
        const $backToList = $('.page-quick-sidebar-back-to-list');
        $backToList.on('click', () => {
            self.selectedUser = new ChatFriendDto();
        });

        const $sidebarTogglers = $('.dropdown-quick-sidebar-toggler a, .page-quick-sidebar-toggler, .quick-sidebar-toggler');
